...
package com.jasper.arcmenudemo.view;
...
import com.jasper.arcmenudemo.R;
import com.jasper.arcmenudemo.util.Util;
...
//    private int mChildPadding = 95;  //卫星与中间的间隔
    private int mChildPadding = 32;  //卫星与中间的间隔
    ...
    private static final int MIN_RADIUS = 95;
    ...
    public ArcLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext=context;
        mChildPadding=Util.dpToPx(mContext,mChildPadding);
        disX=Util.dpToPx(mContext,disX);
...
//        final int size = radius * 2 + mChildSize + mChildPadding + mLayoutPadding * 2;
        final int size = radius * 2  +mChildPadding ;
...
//        int disX= Util.dpToPx(mContext,60);
        final int centerX = (getWidth()+disX) / 2;
     ...
     * 这里太复杂了，个人感觉可以直接用 normalizedDelay=index/childCount 或者 normalizedDelay=（childCount-index+1）/childCount
     */
...
//        int disX= Util.dpToPx(mContext,20);
        final int centerX = (getWidth()+disX)/ 2;
        final int centerY = getHeight() / 2;
        ...
        Interpolator interpolator = mExpanded ? new AccelerateInterpolator() : new OvershootInterpolator(1.0f);
        final long startOffset = computeStartOffset(childCount, mExpanded, index, 0.3f, duration, interpolator);
